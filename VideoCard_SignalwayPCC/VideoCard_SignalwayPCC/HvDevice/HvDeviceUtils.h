...
// 发送执行xml协议命令
bool ExecXmlExtCmd(char* szIP, char* szXmlCmd, char* szRetBuf, int& nBufLen, WORD wAddPort=0);

//发送执行XML协议命令
bool ExecXmlExtCmd(char* szXmlCmd, char* szRetBuf, int& iBufLen, SOCKET sktSend, WORD wAddPort=0);

//获取协议版本
PROTOCOL_VERSION GetProtocolVersion(char* szIP, WORD wAddPort=0);
...
//发送命令
HRESULT HvSendXmlCmd(LPCSTR szCmd, LPSTR szRetBuf, INT iBufLen, INT* piRetLen, DWORD dwXmlVersion, SOCKET sktSend, WORD wAddPort=0);

//通过指定的SOCKET发送Xml命令
HRESULT HvSendXmlCmd(char* szIP, LPCSTR szCmd, LPSTR szRetBuf, INT iBufLen, INT* piRetLen, SOCKET sktSend, WORD wAddPort=0);
//通过指定的SOCKET执行Xml命令
bool ExecXmlExtCmdEx(char* szIP, char* szXmlCmd, char* szRetBuf, int& iBufLen, SOCKET sktSend, WORD wAddPort=0);

//通过指定的SOCKET执行Xml命令（水星协议）
bool ExecXmlExtCmdMercury(char* szIP, char* szXmlCmd, char* szRetBuf, int& iBufLen, SOCKET sktSend, WORD wAddPort=0);
...
HRESULT ExecuteCmd( CHAR* pIP , CAMERA_COMMAND_TYPE eCmdId 
				   ...
				   , CHAR* pRetData , INT* pnRetByte, WORD wAddPort=0);
